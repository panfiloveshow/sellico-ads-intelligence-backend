...
- **Support debug**: Options page can render a scoped evidence report from Sellico backend for real campaign/product/query troubleshooting.
...
6. Open extension **Options** and configure:
   - **Backend URL** (default `https://ads.sellico.ru`)
   - **Email Sellico / Пароль Sellico**, then click **Войти и подключить**
   - **Workspace ID**: optional; leave empty to let the extension pick your active Sellico workspace
   - **Access token**: reserve/development path only; do not use it for normal QA
...
- Bring the stack up locally:
  - `docker compose up -d`
- Sign in through the extension Options page with **Войти и подключить**.
- For local development, you may set Backend URL to `http://127.0.0.1:8080` and use a development access token.
- Workspace is auto-discovered from Sellico when possible; specify it manually only if you need a particular workspace.
...
- **Support evidence debug**: in Options, load a real campaign/product/query report and confirm:
  - `GET /api/v1/extension/evidence-debug/report`
  - readiness/sections/checklist reflect real stored captures only

## Troubleshooting

- **401 / missing authorization header**: token is missing/expired. Open Options and use **Войти и подключить** again.
- **400 missing workspace id**: `Workspace ID` is empty.
- **403 access denied**: token is valid but user is not allowed in that workspace.
- **404 campaign/product not found**: WB page was recognized, but that campaign/product is not synced into this Sellico workspace yet. Run cabinet sync and reload WB.
- **Panel not visible**:
