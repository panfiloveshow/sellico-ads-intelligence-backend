...
RUN --mount=type=cache,target=/go/pkg/mod \
    ...
    CGO_ENABLED=0 GOOS=linux go build -trimpath -buildvcs=false -ldflags="-s -w" -o /out/ozon-cpo-backfill ./cmd/ozon-cpo-backfill
...
COPY --from=builder /out/ozon-cpo-backfill /app/bin/ozon-cpo-backfill
