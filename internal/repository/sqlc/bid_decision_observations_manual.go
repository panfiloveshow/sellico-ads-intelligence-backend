...
func (q *Queries) UpsertBidDecisionObservation(ctx context.Context, arg UpsertBidDecisionObservationParams) error {
	_, err := q.db.Exec(ctx, `
		INSERT INTO bid_decision_observations (
			...
			strategy_binding_id, campaign_id, product_id, wb_campaign_id,
			wb_product_id, placement, old_bid, proposed_bid, reason, metrics,
	...
	`, arg.ObservationKey, arg.WorkspaceID, arg.SellerCabinetID, arg.StrategyID,
		arg.StrategyBindingID, arg.CampaignID, arg.ProductID, arg.WBCampaignID,
		arg.WBProductID, arg.Placement, arg.OldBid, arg.ProposedBid, arg.Reason,
...
func (q *Queries) ListBidDecisionObservationsByStrategy(ctx context.Context, arg ListBidDecisionObservationsByStrategyParams) ([]BidDecisionObservation, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&item.StrategyID, &item.StrategyBindingID, &item.CampaignID, &item.ProductID,
			&item.WBCampaignID, &item.WBProductID, &item.Placement, &item.OldBid,
