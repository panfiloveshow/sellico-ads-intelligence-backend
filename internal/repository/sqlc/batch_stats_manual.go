...
SELECT DISTINCT ON (cs.campaign_id)
  cs.id, cs.campaign_id, cs.date, cs.impressions, cs.clicks, cs.spend, cs.orders, cs.revenue, cs.created_at, cs.atbs, cs.canceled
...
func (q *Queries) GetLatestCampaignStatsBatch(ctx context.Context, workspaceID pgtype.UUID) ([]CampaignStat, error) {
	...
	for rows.Next() {
		var i CampaignStat
		if err := rows.Scan(
			&i.ID, &i.CampaignID, &i.Date, &i.Impressions, &i.Clicks, &i.Spend, &i.Orders, &i.Revenue, &i.CreatedAt, &i.Atbs, &i.Canceled,
		); err != nil {
			return nil, err
		}
