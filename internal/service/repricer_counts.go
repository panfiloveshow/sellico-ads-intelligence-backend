...
func (s *RepricerService) CountUploadTasks(ctx context.Context, workspaceID uuid.UUID) (int64, error) {
	return s.queries.CountPriceUploadTasks(ctx, uuidToPgtype(workspaceID))
}

func (s *RepricerService) CountSchedules(ctx context.Context, workspaceID uuid.UUID, status string) (int64, error) {
	statusFilter := pgtype.Text{}
	if status != "" {
		statusFilter = pgtype.Text{String: status, Valid: true}
	}
	return s.queries.CountPriceSchedules(ctx, uuidToPgtype(workspaceID), statusFilter)
}
