...
### Added — AI-центр Ozon: данные для решений (2026-08-27, вторая волна)
- Остатки и оборачиваемость в контексте ИИ (stock, days_of_cover из
  локальных ozon_product_stocks/ozon_sales_daily) + складской guardrail:
  повышение ставки и включение CPO при остатке < min_stock_for_increase
  отклоняются (неизвестный сток не блокирует).
- Мост к СЕО-сервису: воронка карточки (card_impressions/views/cart,
  conv_to_cart_pct, conv_to_order_pct) из card_metrics_daily через
  internal-экспорт — «чинить ставки vs чинить карточку».
- Мост к коллектору отзывов: shop_rating кабинета + rating/reviews_count
  товаров (матч по названию карточки; у Ozon-отзывов нет ключа SKU).
- Фронт AI-центра: сводка «Кабинет за 7 дней» и мини-статистика кампании
  в шапках групп решений.

### Added — AI-центр Ozon: аудит-фиксы (2026-08-27)
- Guardrail на бюджет кампании без текущего бюджета: предложение ограничено
  2× фактического расхода за окно; без расхода первый бюджет задаёт человек
  (`ozonAIUnbudgetedBudgetGuardReason`).
- `campaign_activate` теперь гейтится потолком «ДРР от общего оборота», а на
  уровне «Автопилот» разрешён только для кампаний, которые ИИ останавливал сам
  (`CountAppliedAIPausesForTarget`).
- Кабинетный дневной лимит применённых ИИ-действий `max_actions_per_day`
  (дефолт 10) — единая точка в `applyProposal`, действует и на автопилот, и на
  подтверждения Копилота.
- TTL 72 ч на proposed-решения: статус `expired`, вычищаются в начале
  ozon:ai_sweep; фронт показывает «Устарело» во вкладке «Отклонено».
- Авто-стоп автопилота: 3 подряд оценённых решения с ухудшением ДРР кампании →
  стратегия переводится на уровень 2 (Копилот) + уведомление владельцам.
- Уведомления в колокольчик CRM (storeExternal): дайджест применённых действий
  после каждого прогона и алерт о даунгрейде (`OzonAINotifier`, best-effort).
- Оценка эффекта cpo_*-решений по per-SKU обороту из `ozon_sales_daily`
  (revenue-only: ДРР/расход NULL, в денежный агрегат не входят).
- «Эффект ИИ»: флаг `low_data` при <5 оценённых решений — фронт показывает
  «пока мало для выводов» вместо метрик.

- Per-SKU расход в контексте ИИ: агрегат из зеркала phrases-отчёта
  (`AggregateOzonSearchSpendBySku`) — отбор SKU в пак теперь по фактическому
  расходу (ставка — tie-break), у товаров появилось поле `spend_30d_rub`.

### Changed
- LLM-провайдер: OpenRouter вместо NVIDIA NIM, только бесплатные модели.
  Основная `z-ai/glm-5.2:free`, фолбэк `nvidia/nemotron-3-super-120b-a12b:free`
  (другой вендор — пулы не пересекаются), текстовый контент (недельные отчёты)
  — `google/gemma-4-31b-it:free` через новый `LLM_CONTENT_MODEL` /
  `ChatRequest.Model` (основная цепочка остаётся фолбэком). Клиент переходит
  на фолбэк не только при 404/402, но и после исчерпания ретраев на 429/5xx —
  общий пул :free-моделей бывает исчерпан часами.
- Дневные лимиты (cooldown/daily caps) считаются по МСК-дню, а не по UTC
  (`ozonStrategyDayStart`).
